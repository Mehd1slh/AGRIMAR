...
def prepare_prompt(address, weather_summary, soil_summary):
    ...
    You are an agriculture expert who answers farmers' questions (crop types, weather, soil, agricultural practices).
    Respond in clear, simple, actionable language suitable for farmers.
    Only answer agricultural-related questions.
    """
...
def CustomChatBot(user_input, prompt_context):
    ...
    response = chatbot.chat.completions.create(
        model="gpt-3.5-turbo",  # swap to 'gpt-4o-mini' if upgrading
        messages=chat
    )
